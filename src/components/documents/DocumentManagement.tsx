  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Documents', value: documentItems.length, icon: FileText, color: 'bg-blue-500' },
          { label: 'Verified', value: documentItems.filter(d => d.status === 'verified').length, icon: CheckCircle2, color: 'bg-green-500' },
          { label: 'Pending', value: documentItems.filter(d => d.status === 'pending').length, icon: Clock, color: 'bg-yellow-500' },
          { label: 'Storage Used', value: '45 MB', icon: Folder, color: 'bg-purple-500' },
        ].map((stat, index) => (
          <Card key={index}>
...
function UploadForm({ onClose, onAdd }: { onClose: () => void; onAdd: (doc: Document) => void }) {
