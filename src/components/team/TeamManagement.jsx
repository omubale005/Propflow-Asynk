...
export function TeamManagement() {
  ...
  } = useLocalEntityStore("team_members", teamMembers);
  ...
  const getMemberStats = (memberId) => {
    ...
    const memberBookings = bookings.filter(
      (b) => b.salesManagerId === memberId,
    );
    const totalRevenue = memberBookings.reduce(
      (acc, b) => acc + b.agreementValue,
      0,
    );
    return {
      ...
      conversionRate:
        memberLeads.length > 0
          ? ((memberBookings.length / memberLeads.length) * 100).toFixed(1)
          : "0",
    };
  };
    ...
    >
      ...
      >
        {[
          ...
          {
            label: "Active",
            value: memberItems.filter((m) => m.status === "active").length,
            icon: Unlock,
            color: "bg-green-500",
          },
          {
            label: "Sales Team",
            value: memberItems.filter((m) => m.role === "sales_executive")
              .length,
            icon: Target,
            color: "bg-orange-500",
          },
              ...
              <div className="flex items-center justify-between">
                ...
                <div
                  className={cn(
                    ...
                    stat.color.replace("bg-", "bg-opacity-10 "),
                  )}
                >
                  <stat.icon
                    className={cn(
            ...
            >
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-3 gap-4 pt-4 border-t border-gray-100">
                  ...
                  <div className="text-center">
                    <p className="text-lg font-bold text-[#0082f3]">
...
function MemberDetails({ member }) {
  const stats = {
    ...
    revenue: bookings
      .filter((b) => b.salesManagerId === member.id)
      .reduce((acc, b) => acc + b.agreementValue, 0),
    siteVisits: 45,
    callsMade: 320,
  };
    ...
    <div className="space-y-6 py-4">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <Badge
            className={cn("text-sm capitalize mt-1", roleColors[member.role])}
          >
            {member.role.replace(/_/g, " ")}
          </Badge>
...
function AddMemberForm({ onClose, onAdd }) {
